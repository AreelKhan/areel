---
title: "So are LLMs thinking?"
description: "Hand-wavy explanations of how LLMs respond"
date: "2023-05-12"
tags: ["LLMs"]
readTime: "8 min"
featured: true
...
import AutoRegressionDemo from "@components/blog-assets/AutoRegressionDemo.astro";

A few weeks ago, while sitting around a campfire (and being destroyed by mosquitoes), my friends and I attempted to define what it means to _think_. Along the way, a stepping stone we first tried to answer was "Are LLMs thinking?"

My opinion was that no, LLMs are not thinking. Despite not yet having defined "thinking", I was comfortable stating that whatever LLMs do is not that. In retrospect it was more of an emotional opinion than a logical one.

My reasoning was that LLMs merely predict the next word, and that doesn't _feel_ like thinking.

My friends are not familiar with how an LLM works. Hence, the argument did not stick. They (reasonably) could not wrap their heads around how something that could write their history essay was "just predicting the next word".

I was unable to explain auto-regression with literal sticks and stones. I can't believe I am saying this, but I wish we had been in a classroom instead of a beach.

Regardless, if I were to try to explain how an LLM generates responses, I'd go about it something like this.

### Predicting the _next_ word
LLMs (like ChatGPT) simply predict the __next word__ in a sentence.
...
<video controls style={{ width: "100%", margin: "2rem 0" }}>
  <source src="/videos/AutoRegressionExample.mp4" type="video/mp4" />
...
At this point, the LLM doesn't know what the rest of its answer is going to be, it just _thinks_ that `A` is the best way to start<sup>[2](#f2)</sup>.
...
The LLM has no plan for the full sentence. It's not drafting a response in its head and then revealing it to you word by word. It is _literally just predicting the next word_.  This is exactly the behavior that makes me feel that LLMs are not thinking. But again, I haven't even decided what thinking means.

---

### Well, not _just_ the next word

I've actually been lying. I've been saying `LLMs predict the next word`. But that's not the whole truth. The LLM doesn't just tell you the most likely next word. It returns to you how likely it is that each word it knows will be next word.

A visualization will make more sense:

<video controls style={{ width: "100%", margin: "2rem 0" }}>
  <source src="/videos/TokenProbabilityDistributionExample.mp4" type="video/mp4" />
  Your browser does not support the video tag.
</video>
...
<a id="vocabulary"></a>
An important confession: I was lying yet again. LLMs don't predict words at all. In fact, LLMs probably don't even know what words are. Instead LLMs predict _tokens_. Tokens are like pieces of words; think of them as syllables. I am introducing tokens now as opposed to earlier to keep explanations digestable. I promise I'm not a chronic liar.

This is what it actually looks like:

<video controls style={{ width: "100%", margin: "2rem 0" }}>
  <source src="/videos/TokensNotWords.mp4" type="video/mp4" />
  Your browser does not support the video tag.
</video>
...
Since words and tokens are all the same to LLMs, its more efficient to use tokens <sup>[7](#f7)</sup>.
...
3. <a id="f3"></a> I used the phrase "most likely response" in handwavy fashion so it could mean many things. Within the realm of autoregressive models, here's two definitions I could think of. First, the response generated by greedily sampling the most likely token at each step:

$$
x = (x_1, x_2, \ldots, x_n), \quad x_i = \arg\max_{w \in V} \; P(y_i = w \mid y_{\lt i})
$$

  Second, the response that maximizes the total product of probabilites for each token:

$$
x^* = \arg\max_{x \in V^n} \; \prod_{i=1}^{n} P(x_i \mid x_{\lt i})
$$

4. <a id="f4"></a> If my understanding is correct, theoretically, this is only source of randomness in the LLM. If you always picked the most likely word, you'd get the same response every time. But floating points are not perfectly accurate. So I don't know.
...
7. <a id="f7"></a> A good question at this point is: forget tokens, why not use the alphabet? Then we can create any word and the LLM only memorizes the alphabet. The problem with this is that the sequence to predict gets too long and since character level tokens encode very little information, the model performance drops. I should elaborate on this. But to be honest, I don't fully know myself.
