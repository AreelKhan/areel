...
TOKEN_DISTRIBUTIONS = [
    [("A", 0.43), ("Th", 0.18), ("It", 0.12), ("In", 0.07)],
    [("ca", 0.51), ("ve", 0.20), ("en", 0.12), ("au", 0.06)],
    [("r", 0.48), ("mp", 0.23), ("ll", 0.18), ("pt", 0.08)],
    [("wo", 0.39), ("th", 0.26), ("us", 0.18), ("wi", 0.10)],
    [("rks", 0.39), ("ol", 0.26), ("th", 0.18), ("ep", 0.10)],
]
...
class TokenProbabilityDistributionExample(Scene):
    def construct(self):
        ...
        for index, distribution in enumerate(TOKEN_DISTRIBUTIONS):
            ...
            self.play(FadeOut(label_input), run_time=EXPLANATION_FADE_TIME * 0.5)
            ...
            self.play(FadeIn(label_thinking), run_time=EXPLANATION_FADE_TIME * 0.5)
            ...
            label_predict = Text(
                "The LLM returns probabilities for next token",
            ...
            token_at_input.shift(LEFT * 0.05)

            label_pick = Text("Pick the most likely next token", font_size=EXPLANATION_FONT_SIZE)
            ...
            new_response = Text(responses_over_time[index], font_size=FONT_SIZE)
